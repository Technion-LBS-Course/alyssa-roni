...
# רצפת-בנד תחתונה פר-רמה: הגבול התחתון של בנד-העיקוף. אם המסלול המוצל-ביותר שנמצא
# בתקציב עדיין קצר מ-(רצפה × המהיר) — נופלים למסלול המהיר עם הודעה מפורשת
# (`little_shade_below_band`), במקום "מסלול מוצל" סתום שבפועל זהה למהיר.
# שתי סמנטיקות שונות מאחורי אותו מנגנון (לא לבלבל):
# "מעט צל" (2026-07-19, החלטת-מוצר של המשתמשת): רף-**כדאיות** — מתחת ל-10% עיקוף
# נחשב "לא שווה תכונה נפרדת", גם אם טכנית נמצא הבדל אמיתי כלשהו.
# "הרבה צל" (2026-07-19 מאוחר יותר): רף-**רעש** נמוך-נמוך — תופס רק את המקרה
# הדגנרטיבי שבו גם בחיפוש הכי-אגרסיבי-לצל לא נמצא שום מסלול שונה מהותית (למשל
# בשעות שמש-נמוכה שבהן כל העיר כבר בצל, ואין שום עיקוף שיכול לשפר TCI שכבר בשפל).
# זה **לא** שיפוט-כדאיות כמו ב"מעט צל" — רק זיהוי "אין מה להראות", כדי לא להציג
# בשקט השוואת-מסלולים זהה כאילו נמצאה בחירה משמעותית.
_TIER_MIN_RATIOS = {
    1.0: 1.10,   # מעט צל — רף-כדאיות מוצרי
    2.0: 1.02,   # הרבה צל — רף-רעש (לא רף-כדאיות)
}
...
_DID_YOU_KNOW = (
    ...
    "הידעת? אחד מכל 5 אנשים בארה\"ב יפתח סרטן עור במהלך חייו.",
    ...
    "הידעת? הגנה הפוכה: ג'ינג'ים מייצרים סוג מלנין שלא מגן מקרינה, אלא עלול אפילו להגביר את נזקי השמש.",
    "הידעת? נשרפים גם בצל חלקי: בעלי עור בהיר מאוד נוטים להישרף גם בימים מעוננים או בשעות אחר הצהריים המאוחרות.",
    "הידעת? רגישות יתר לחום: מחקרים מראים שג'ינג'ים רגישים יותר לשינויי טמפרטורה וחשים כאב של כוויות שמש בעוצמה גבוהה יותר.",
    "הידעת? אפס הגנה טבעית: אצל לבקנים הגוף לא מייצר מלנין כלל, ולכן העור חסר הגנה לחלוטין מפני קרינת UV.",
    "הידעת? סכנה לעיניים: לבקנות גורמת לרגישות קיצונית לאור (פוטופוביה) — מסלול מוצל שומר גם על הראייה שלהם מהסתנוורות.",
    "הידעת? סיכון מקסימלי: בשל המחסור במלנין, כל דקה בשמש היא סכנה מיידית לכוויות קשות וגורם סיכון עליון לסרטן העור.",
)
...
def plan_route(
    ...
    *,
    ...
    if shade_r in _TIER_MIN_RATIOS:
        route_floor = _route_at(LAMBDA_FLOOR) or route_short
        floor_ratio = route_floor["distance_m"] / base_dist
        min_ratio = _TIER_MIN_RATIOS[shade_r]
    ...
    if shade_r in _TIER_MIN_RATIOS and route1_ratio < _TIER_MIN_RATIOS[shade_r]:
        fast_route = compute_length_route(
            origin_latlon, dest_latlon, tci_by_uv=tci_uv, G=graph)
        result["route_result"] = fast_route
        result["fallback"] = "little_shade_below_band"
