...
5. **Graph Routing Algorithm (NetworkX / A\*):** A* עם היוריסטיקת Haversine — מוצא מסלול עלות מינימלית במשקל **אדיטיבי** `(TCI-1) × length × street_factor + λ × length` (איבר-שמש מצטבר + מחיר-מרחק; λ נבחר בחיפוש בינארי לפי תקציב-העיקוף של הרמה). Fallback ל-OSRM בלילה/עננות/מודל חסר.
...
**`shade_factor` אינו אקספוננט יותר** (מ-2026-07-19) — הוא בורר לאיזה **תקציב-עיקוף** לכוון. משקל הקשת הוא **אדיטיבי**: `(TCI-1) x length x street_factor + λ x length` (איבר-שמש שמצטבר על החום לאורך הקשת + מחיר-מרחק; λ נמצא בחיפוש בינארי כדי למלא את תקציב-העיקוף בצל). המודל האדיטיבי מבטיח מונוטוניות ב-λ, ולכן ההיררכיה בין הרמות לעולם לא מתהפכת (החליף את `TCI^shade_factor` הלא-מונוטוני).
...
- **תקציב-עיקוף (`_TIER_TARGET_RATIOS`, מודל אדיטיבי מ-2026-07-19):** המסלול המוצל שואף לנצל עד ~1.30× ("מעט צל") / ~1.60× ("הרבה צל") מאורך המסלול הישיר **כדי לקנות צל** — `plan_route` מוצא בחיפוש בינארי (λ∈[0.05,50], 24 איטרציות) את ה-λ הקטן ביותר שעדיין בתוך התקציב. מחליף את תקרת ה-`DETOUR_CAP=1.6` + הורדת-`shade_factor` הישנה (שהיו לא-מונוטוניות וגרמו ל"מעט צל" לצאת לפעמים ארוך/חם מ"הרבה צל")
- **Pedestrian bonus:** footway/pedestrian/path/living_street — משקל x0.8
- **TCI color gradient על המסלול:** ירוק<4, כתום 4-7, אדום>7
- **Fallback modes:** model_missing / night / overcast / weights_missing → OSRM אוטומטי; `little_shade_below_band` (רק "מעט צל" — גם המסלול הכי-מוצל-אפשרי מתחת ל-1.10× מהישיר) → המסלול הקצר-ביותר על הגרף; `shade_gain_not_visible` / `same_as_little_shade` → מוצג המסלול הקצר/פשוט יותר כשאין שיפור-TCI נראה-לעין
