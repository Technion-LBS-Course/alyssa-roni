...
`canopy_ratio` הוסר מהנוסחה (הוא מדד את אותה הצללה בצורה סטטית וכפולה), ונשאר כפיצ'ר-decoy מכוון.

`shadow_cov` ∈ [0,1] הוא אחוז המקטע המכוסה בצל **מבנים ועצים כאחד**, מחושב מ**מצולעי צל אמיתיים**. כל מבנה/עץ מטיל צל בכיוון ההפוך לשמש, באורך:

```
shadow_length = h / tan(sun_altitude)
```

- **מבנים** — footprint שגודלו וכיוונו נגזרים מהצפיפות המקומית ומהרחוב הקרוב.
- **עצים** — מצולע חופה אמיתי, בגובה משוער לפי `area_class`.

לבסוף בודקים איזה חלק מהמקטע נחתך עם איחוד כל הצללים.

**למה זה שיפור:** הנוסחה הישנה תפסה רק זווית, לא גאומטריה:

```
shadow_old = building_height × sin(shadow_angle)
```

ולכן זקפה הצללת-שווא ברחובות מזרח-מערב (כשהשמש מקבילה לרחוב). ר' §11 לפירוט ההתפתחות והאימות מול Shadowmap.
...
flowchart TD
    ...
    subgraph Routing["אלגוריתם ניווט (Routing Layer)"]
        I["גרף משוקלל: A* עם משקל אדיטיבי (TCI-1)×אורך×factor + λ×אורך"]
    end
...
5. **Graph Routing Algorithm (NetworkX / A\*):** A* עם היוריסטיקת Haversine מוצא מסלול בעלות מינימלית, במשקל **אדיטיבי** לכל קשת (ראו הנוסחה למטה). λ נבחר בחיפוש בינארי לפי תקציב-העיקוף של הרמה, ויש Fallback ל-OSRM בלילה/עננות/מודל חסר.

משקל הקשת ב-A\* — איבר-שמש מצטבר ועוד מחיר-מרחק:

```
weight = (TCI-1) × length × street_factor  +  λ × length
```
...
| 2 | **נתונים** | **Spatial Mismatch** — חוסר התאמה גיאומטרי מובנה בין רשת הדרכים של OSM לשכבות ה-GIS של העירייה ומפ"י. | בינונית | ביצוע שיוך מרחבי מבוסס רדיוס השפעה ב-GeoPandas (`Buffer-radius` נפרד: 25מ' למבנים מצנטרואידים, 10מ' לעצים בחיתוך פוליגונים), המבטיח הצלבה נכונה של המבנים והעצים לרחוב גם תחת סטיות מיפוי. |
...
> **מאז ש-`shadow_cov` מאחד צל-מבנים וצל-עצים לאות פיזי אחד, הוא נושא כמעט את כל האינפורמציה הרלוונטית.** שאר הפיצ'רים נדחקו למטה: `canopy_ratio` ו-`building_height` הפכו ל-decoys אמיתיים (`shadow_cov` בלע את שני התפקידים הפיזיים שלהם), וגם `temperature` (decoy) נחלש ככל ש-`shadow_cov` מסביר יותר מהשונות.

שינוי החשיבות אחרי איחוד הצל:

| פיצ'ר | לפני | אחרי |
|--------|------|------|
| `shadow_cov` | 0.072 | **0.764** |
| `sun_altitude` | 0.747 | 0.168 |
| `temperature` (decoy) | 0.111 | 0.031 |
| `canopy_ratio` / `building_height` | — | 0.000 |
...
1. **footprint מבנים לפי צפיפות+כיוון:** גודל הריבוע נגזר מהצפיפות המקומית — ממוצע המרחק ל-5 השכנים הקרובים, מוגבל לטווח 5–20 מ' (`clip[5,20]`). כיוונו מיושר לרחוב הקרוב ביותר, במקום ריבוע קבוע וישר-צירים.
2. **צל-עצים אוחד ל-`shadow_cov`:** מצולעי חופה אמיתיים (לא ריבוע!) נגררים לפי גובה-עץ משוער — buckets של 3–14 מ' לפי `area_class`. הם מסוננים אדפטיבית-לשעה (רק עצים שיכולים להגיע לרחוב בזווית-השמש הנתונה) ומאוחדים לאותו STRtree עם צללי המבנים. האיחוד (`unary_union`) פותר double-counting אוטומטית.

**תוצאה מדודה:** בשדרות עם חופת-עצים מתועדת (רוטשילד 24.2%, ח"ן 33.6%, בן גוריון 26.1%), חציון ה-`shadow_cov` בצהריים עלה מ-0.000 ל-0.20–0.30 — הצללת-הצהריים מהעצים סוף-סוף נתפסת. במקביל `canopy_ratio` ו-`building_height` צנחו לחשיבות 0.000 (decoys אמיתיים), ו-`shadow_cov` עלה ל-0.764.
...
3. בחרו **רמת צל** — **2 רמות**: 🚶 מעט צל (ברירת מחדל) / 🌳 הרבה צל — ממופה ל-`shade_factor` (1.0/2.0) שבוחר את **תקציב-העיקוף** (1.30× / 1.60× מהמסלול הישיר)
...
6. **A\*** (היוריסטיקת Haversine × λ, admissible) מוצא את המסלול בעל עלות מינימלית במשקל **אדיטיבי** `(TCI-1) × length × street_factor + λ × length`; `plan_route` מוצא ב**חיפוש בינארי** את ה-λ הקטן ביותר שנותן מסלול בתוך **תקציב-העיקוף** של הרמה (~1.30× ל"מעט צל" / ~1.60× ל"הרבה צל" מהמסלול הישיר)
...
- **אופק תאריכים (7 ימים):** תאריך מחוץ ל-`[today, today+7]` (או תאריך-עבר) → שגיאה "מזג האוויר זמין רק ל-7 הימים הקרובים" (במקום נפילה שקטה להיום). נאכף דטרמיניסטית ב-`_validate`.
- **אדישות-לצל = מהיר:** "לא אכפת לי מהצל"/"לא משנה לי צל" → `mode=fast` (המשתמש לא רוצה צל → אין טעם לעקוף בשבילו).
...
> **ולידציית מספר-בית + תיקון Photon (2026-07-18):** כשיש מספר בית בכתובת, תוצאה מ-Nominatim/Photon שנפלה בשקט לרמת-רחוב (בלי אותו מספר-בית בדיוק, למשל "יפת 4900") נדחית במקום להתקבל כמדויקת. בנוסף הוסר פרמטר `lang="he"` שבור ב-Photon (הוא תומך רק ב-default/de/en/fr) — שבלעדיו כל שלב-הגיבוי השלישי נדחה בשקט ומעולם לא עבד. כשמספר-הבית לא קיים אך הרחוב כן — נפילה **מפורשת** למרכז-הרחוב.

### shade_level — שליטה על עוצמת העדפת הצל

**2 רמות** (עודכן מ-3 ל-2 ב-2026-07-18; "balanced" הוסרה כי כמעט תמיד התכנסה לאותו מסלול כמו "max"):

| רמה | shade_factor | תקציב-עיקוף | אפקט |
|-----|-------------|-------------|-------|
| short (🚶 מעט צל) | 1.0 | 1.30× | ברירת מחדל; מוטה למסלול קצר, צל שיקול משני |
| max (🌳 הרבה צל) | 2.0 | 1.60× | מקסימום צל; מוכנות לעקיפה ניכרת |

**`shade_factor` אינו אקספוננט יותר** (מ-2026-07-19) — הוא בורר לאיזה **תקציב-עיקוף** לכוון. משקל הקשת הוא כעת **אדיטיבי**:

```
weight = (TCI-1) × length × street_factor  +  λ × length
```

האיבר הראשון הוא עלות-שמש שמצטברת לאורך הקשת, והשני הוא מחיר-מרחק. λ נמצא בחיפוש בינארי כדי למלא את תקציב-העיקוף בצל. המודל האדיטיבי מבטיח מונוטוניות ב-λ, ולכן ההיררכיה בין הרמות לעולם לא מתהפכת — בניגוד ל-`TCI^shade_factor` הישן והלא-מונוטוני.
...
- **A\* במקום Dijkstra:** היוריסטיקת Haversine × λ (admissible) — מהיר יותר עם אחריות אופטימליות
- **תקציב-עיקוף** (`_TIER_TARGET_RATIOS`, מודל אדיטיבי מ-2026-07-19): המסלול המוצל שואף לנצל תקציב-עיקוף מהמסלול הישיר **כדי לקנות צל**:
  - "מעט צל" → עד ~1.30× מאורך המסלול הישיר.
  - "הרבה צל" → עד ~1.60×.

  `plan_route` מוצא בחיפוש בינארי (24 איטרציות, λ בטווח 0.05–50) את ה-λ הקטן ביותר שעדיין בתוך התקציב. זה מחליף את המנגנון הישן (תקרת `DETOUR_CAP=1.6` + הורדת `shade_factor`), שהיה לא-מונוטוני וגרם ל"מעט צל" לצאת לפעמים ארוך/חם יותר מ"הרבה צל".
- **Pedestrian bonus:** footway/pedestrian/path/living_street — משקל x0.8
- **TCI color gradient על המסלול:** ירוק<4, כתום 4-7, אדום>7
- **Fallback modes** — מצבים שבהם הניווט המוצל נסוג אוטומטית:
  - `model_missing` / `night` / `overcast` / `weights_missing` → חזרה ל-OSRM.
  - `little_shade_below_band` (רק "מעט צל" — גם המסלול הכי-מוצל-אפשרי מתחת ל-1.10× מהישיר) → המסלול הקצר-ביותר על הגרף.
  - `shade_gain_not_visible` / `same_as_little_shade` → מוצג המסלול הקצר/פשוט יותר, כשאין שיפור-TCI נראה-לעין.

### בדיקות (Testing) — ממופה לשקופיות הקורס ("green != real")

הטסטים בנויים לפי **מודל היהלום** ו-4 המסלולים המפחידים: לא לרדוף אחרי כיסוי מלא, אלא לבדוק שהמערכת **מגיבה נכון לשינוי** ועומדת בסיכון המרכזי — טסט חזק מאמת **התנהגות/ערך**, לא רק "מחזיר משהו".

```bash
# שכבה דטרמיניסטית (74 טסטים, בלי מפתח/רשת):
python -m pytest tests/ --ignore=tests/test_agent_llm.py -v
# שכבת LLM חיה (14 טסטים, דורשת מפתח; מדלגת בלי מפתח):
GROQ_API_KEY=... python -m pytest tests/test_agent_llm.py -v
```

| קובץ | שכבה | מה מוכיח |
|------|------|----------|
| `test_smoke.py` | בסיס | import + טעינת מודל + **predict אמיתי** (לא רק שהקובץ נפתח) |
| `test_agent.py` | Unit | ולידציה/coercion, כלל-לילה, טבלת-תאריכים, fallback לתובנה (ללא רשת) |
| `test_agent_scenarios.py` | Unit | ולידציה דוחה זבל, שגיאת תאריך-רחוק, גבולות כלל-לילה, קלט-קצה |
| `test_model_contract.py` | Integration | **המודל מגיב לשינוי** — מונוטוניות (צל/עננות/שמש) + אדישות ל-decoys (מודל "תמיד-5" נכשל) |
| `test_route_flow.py` | Integration | גרף סינתטי מבוקר: **הצל משנה את בחירת המסלול**, המשקלים **נכנסים ל-A\***, אינvariantים אמיתיים (קצוות/אורך/תיבת-ת"א) |
| `test_route_invariants_live.py` | Integration (נתונים אמיתיים) | **מונוטוניות ללא היפוך** על מסלולים אמיתיים: מהיר ≥ מעט צל ≥ הרבה צל ב-TCI, ו-≤ במרחק (רשת-ביטחון למודל האדיטיבי) |
| `test_shade_gate_regressions.py` | Integration | שערי-הצל האדיטיביים (`little_shade_below_band` / `shade_gain_not_visible` / `same_as_little_shade`) על מסלולים אמיתיים |
| `test_insights.py` | Unit | תובנות מסלול + תיקון שגיאת-כתיב ברחוב |
| `test_geocode_house_number.py` | Unit | ולידציית מספר-בית (Nominatim+Photon) + תיקון פרמטר `lang` שבור ב-Photon |
| `test_geocode_retry.py` | Unit | ניסיון-חוזר על כשל-רשת חולף (429/5xx), לא על "לא נמצא" לגיטימי |
| `test_snap_tci.py` | Unit | הצמדת TCI פר-מקטע למסלול חיצוני (מרחק-סף, ספירת חשיפה גבוהה) |
| `test_agent_llm.py` | Integration חי | הבנת ה-LLM מול Groq אמיתי — כל השדות הקריטיים, וריאציות ניסוח, קלט מבולגן. מודל קטן דרך `SHADY_TEST_MODEL` לחיסכון-מכסה; rate-limit → skip (לא fail) |

> **שני ממצאים שהטסטים תפסו:** (1) הבחנה בין 429 (rate-limit, מדולג) לכשל-הבנה אמיתי; (2) "לא אכפת לי מהצל" התפרש כ-shaded גם ב-70b — ציפיית-טסט שגויה, שהובילה להחלטה המוצרית שאדישות = מהיר.
